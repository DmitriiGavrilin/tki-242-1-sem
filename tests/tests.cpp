...
TEST(VariantOperations, FirstTransformation) {
    Task1 task(makeMatrix({{3, 3, 1}, {-2, -1, -1}}));
    task.solve();
    expectMatrix(task.getMatrix(), makeMatrix({{0, 0, 1}, {-2, 0, 0}}));
}

TEST(VariantOperations, SecondTransformation) {
    Task2 task(makeMatrix({{3, 1}, {2, 2}, {-6, 5}}));
    task.solve();
    expectMatrix(task.getMatrix(), makeMatrix({{0, 0}, {3, 1}, {2, 2}, {0, 0}, {-6, 5}}));
}
